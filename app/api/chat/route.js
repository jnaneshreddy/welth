...
// Function to list available models via direct API call
...
export async function POST(req) {
  try {
    ...
    if (availableModels.length === 0) {
      return new Response(
        JSON.stringify({ 
          error: "No models available. Please check your API key permissions.",
          debug: "Your API key might not have access to Gemini models, or the key might be invalid."
        }),
    ...
    // Build contents array for multi-turn context
    const contents = [];
